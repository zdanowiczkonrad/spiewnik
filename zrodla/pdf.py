# -*- coding: utf-8 -*-
# Śpiewnik wyjazdowy — WERSJA Z CHWYTAMI + porządek nabożeństw (A4).
import os, re
...
for t in SELECTED:
    s=BYALL.get(t)
    if not s or s["stub"]: print("[POMINIĘTO — brak/stub w bazie]",t); continue
    content.append(s)
for i,s in enumerate(content,1): s["nr"]=i
BYT={s["title"]:s for s in content}

class PDF(SongbookPDF):
    left_label="ŚPIEWNIK WYJAZDOWY"; right_label=EVENT["header"]
...
pdf.ln(3)
pdf.line(105-22,pdf.get_y(),105+22,pdf.get_y()); pdf.ln(8)
...
for dzien,uroczystosc,charakter,czyt,mysl,sloty in PLAN:
    ...
    for rola,piesn,opis in sloty:
        ...
        pdf.set_xy(ML+34,ytop); pdf.set_font("play","B",10.5); pdf.set_text_color(*INK)
        pdf.multi_cell(PW-34,5.2,piesn,new_x=XPos.LMARGIN,new_y=YPos.NEXT)
...
pdf.set_x(ML); pdf.set_font("play","B",10.5); pdf.cell(0,5.2,"Apel Jasnogórski",new_x=XPos.LMARGIN,new_y=YPos.NEXT)
...
def song_metrics(s):
    ...
    bs=9.5 if maxw<=0 else max(6.0, min(9.5, 10.0*PW/maxw))
    lh=bs*0.46
    return lines,bs,lh,17+lh*len(lines)+7
...
def draw_song(s):
    lines,bs,lh,need=song_metrics(s)
    ...
    meta=[]
    if s.get("key"): meta.append("tonacja "+fmt_key(s["key"]))
    if meta:
        pdf.set_x(ML+sq+gap); pdf.set_font("sans","",7.8); pdf.set_text_color(*GRY); cspace(0.8)
        pdf.multi_cell(PW-sq-gap,4,"  ·  ".join(meta).upper(),new_x=XPos.LMARGIN,new_y=YPos.NEXT); cspace(0)
    ...
    for l in lines:
        ...
        pdf.set_text_color(*INK); pdf.ln(lh)
    pdf.ln(7)
...
out=os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                 f"Śpiewnik wyjazd – {EVENT['short']}.pdf")
pdf.output(out)
